...
function express() {
      ...
      if (idx < middlewares.length) {
        ...
        if (mw.path) {
          if (req.path.startsWith(mw.path)) {
            return mw.handler(req, res, next);
          }
          return next();
        }
  ...
  const app = {
    ...
    get(pathname, handler) {
      routes.GET.push({ path: pathname, handler });
    },
    post(pathname, handler) {
      routes.POST.push({ path: pathname, handler });
    },
    put(pathname, handler) {
      routes.PUT.push({ path: pathname, handler });
    },
    delete(pathname, handler) {
      routes.DELETE.push({ path: pathname, handler });
    },
    patch(pathname, handler) {
      routes.PATCH.push({ path: pathname, handler });
    },
    set(key, value) {
      settings[key] = value;
    },
